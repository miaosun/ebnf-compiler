...
class SimpleNode implements Node {
  ...
  public static void addToken(String i){
    ...
    if(m.matches())
    { //Nome nao é simbolo
      // isto ta comentado pk troquei os if's pk a regex ta com um bug ou o caralho -.-
    ...
    {
      // isto ta comentado pk troquei os if's pk a regex ta com um bug ou o caralho -.-
      /*System.out.println("Not Matched: passei aqui a escrever : "+testStr);
      testStr = testStr.trim().replace(" ", "_");
      RandomString str = new RandomString(4);
      String string = str.nextString();
      if(!symbolNames.contains(string))
  ...
  public String getTokens()
  {
    String tokens = "";
    int j = 0;
    for(int i = 0 ; i < tokensJavaCC.size() ; i++)
    {
      Pattern p = Pattern.compile("[^a-zA-Z0-9\\s]*"); //wtfomgbbq pk é que isto está a fazer match de NAO PALAVRAS quando deveria ser DE PALAVRAS so????
      Matcher m = p.matcher(tokensJavaCC.get(i));    
      if(m.matches())
      { 
      ...
      {
        if(tokensJavaCC.get(i).contains("."))
        {
          if(i != tokensJavaCC.size()-1)
            tokens+="< "+tokensJavaCC.get(i).trim().replace(".","")+" : \""+tokensJavaCC.get(i)+"\" > | \n";
          else
            tokens+="< "+tokensJavaCC.get(i)+" : \""+tokensJavaCC.get(i)+"\" >";
        }
        else
        {
          if(i != tokensJavaCC.size()-1)
